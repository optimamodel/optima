...
def optimize(D, objectives=None, constraints=None, ntimepm=1, timelimit=60, verbose=2):
    ...
    def objectivecalc(optimparams):
        ...
        thisalloc = timevarying(optimparams, ntimepm=ntimepm, nprogs=nprogs, t=D.opt.simtvec, totalspend=totalspend)        
        newD = deepcopy(D)
    ...
    for i, params in enumerate([origalloc, optparams]):
        alloc = timevarying(params, ntimepm=len(params)/nprogs, nprogs=nprogs, t=D.opt.simtvec, totalspend=totalspend)            
