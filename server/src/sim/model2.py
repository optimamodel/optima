...
def model2(G, M, F, options, verbose=2): # extraoutput is to calculate death rates etc.
    """
    MODEL
    ...
    eff_circ = M.const.eff.circ*M.circum
    eff_stiprevulc = M.const.eff.sti*M.stiprevulc
    mmi = M.const.trans.mmi
    mfi = M.const.trans.mfi
    mmr = M.const.trans.mmr
    mfr = M.const.trans.mfr
    const_eff_condom = M.const.eff.condom
    const_trans_inj = M.const.trans.inj
    pships_inj = M.pships.inj
    pships = M.pships
        ...
        for popM in range(G.npops):
            for popF in range(G.npops):
                ...
                for act in ['reg','cas','com']:
                    if pships[act][popM,popF]>0:
                        circeff = 1 - eff_circ[popM,t] # Effect of circumcision
                        stieffM = 1 + eff_stiprevulc[popM,t] # STI prevalence effect
                        stieffF = 1 + eff_stiprevulc[popF,t] # STI prevalence effect
                        transM = mmi if G['male'][popF] else mfi
                        transF = mmr if G['male'][popF] else mfr

                        condomprob = (M['condom'][act][popM,t] + M['condom'][act][popF,t]) / 2 # Reconcile condom probability
                        condomeff = 1 - condomprob*const_eff_condom; # Condom use
                        forceinfM = 1 - math.pow((1-transM*circeff*stieffM),(dt*M['totalacts'][act][popM,popF,t]*condomeff*effhivprev[popF])) # The chance of person B infecting person A
                        forceinfF = 1 - math.pow((1-transF*circeff*stieffF),(dt*M['totalacts'][act][popF,popM,t]*condomeff*effhivprev[popM])) # The chance of person B infecting person A
...
#        metheff = 1 - M.const.eff.meth*M.ost[t] # TODO: methadone should be subtracted from population size
                if pships_inj[popM,popF]>0:
                    numacts1 = M['sharing'][t] * M['totalacts']['inj'][popM,popF,t] / 2 # Number of acts per person per year -- /2 since otherwise double-count# TODO
                    numacts2 = M['sharing'][t] * M['totalacts']['inj'][popF,popM,t] / 2 # Number of acts per person per year
                    forceinf1 = 1 - math.pow((1-const_trans_inj),(dt*numacts1*effhivprev[popF])) # Force of infection
                    forceinf2 = 1 - math.pow((1-const_trans_inj),(dt*numacts2*effhivprev[popM])) # Force of infection
                    forceinfvec[popM] = 1 - (1-forceinfvec[popM]) * (1-forceinf1) # Calculate the new "male" forceinf, ensuring that it never gets above 1
                    forceinfvec[popF] = 1 - (1-forceinfvec[popF]) * (1-forceinf2) # Calculate the new "male" forceinf, ensuring that it never gets above 1
