...
#        simboxdict['BOC_objectives'] = self.BOCy
    ...
    def createsimopt(self, sim, optalloc, optobj, newbudget):
        if not sim == None:
            print('Converting optimisation results into a new budget simulation...')
            self.simlist.append(SimBudget(sim.getname(), self.getregion(), newbudget))
    ...
    def runallsims(self, forcerun = False, forceopt = False):
        ...
        for sim in self.simlist:
            ...
            if sim.isprocessed() and (forceopt or not sim.isoptimised()):
                (optalloc, optobj, newbudget, makenew) = sim.optimise()
                tempsim = sim
        ...
        if makenew:
            self.createsimopt(tempsim, optalloc, optobj, newbudget)
