...
4. Choose a name for your project and then follow the ensuing instructions.
...
- However, ALL processes should be initiated at the Project class level (projectclass.py) and propagate down.
  'Project' should ideally be hooked up as the interface for the front-end.
  For example, want to apply 'optimize' to the nth loaded region within the project?
  Code something like 'def optimize(self,n,parameters)', which should in turn call 'self.regionlist[n-1].optimise(parameters)'.
- Likewise, any processes to apply across multiple Ds should be coded while iterating through a subset of Project's regionlist.
