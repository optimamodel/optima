def setoptions(opt=None, **kwargs):
    ...
    if not(isinstance(opt,dict)): 
        ...
        opt['dt'] = kwargs.get('dt', 0.2) # Timestep
        opt['nsims'] = kwargs.get('nsims', 5) # Number of simulations to store for purposes of uncertainty
        opt['quantiles'] = kwargs.get('quantiles', [0.5, 0.25, 0.75]) # Quantiles to return
        opt['growth'] = kwargs.get('growth', 0.0) # Default population growth rate
        opt['disc'] = kwargs.get('disc', 0.05) # Economic discounting rate
