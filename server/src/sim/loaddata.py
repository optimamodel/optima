...
def loaddata(filename='./epi-template.xlsx',verbose=True):
    ...
    for sheettype,datanames in enumerate([basicdata, matrices, constants]): # Loop over each type of data, but treat constants differently
        for j in range(len(datanames)): # Loop over each spreadsheet for that data -- just one for constants
            if verbose: print('  Loading "%s"...' % sheetnames[sheettype][j])
            ...
            sheetdata = spreadsheet.sheet_by_name(sheetnames[sheettype][j]) # Load this spreadsheet
            ...
            for r in range(sheetdata.nrows): # Loop over each row in the spreadsheet
                ...
                if len(paramcategory): # It's not blank: e.g. "HIV prevalence"
                    ...
                    if len(subparam): # The subparameter name isn't blank, load something!
                        ...
                        if sheettype==0: # It's basic data, append the data and check for programs
                        ...
                        if sheettype==1: # It's a matrix, append the data 
                            data[name][thispar].append(thesedata) # Store data
                                                        
                        if sheettype==2: # It's a constant, create a new dictionary entry
