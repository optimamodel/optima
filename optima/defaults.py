...
def defaultprograms(project, addcostcovpars=False, addcostcovdata=False, filterprograms=None):
    ...
    if addcostcovpars:
        year = 2016.
        Condoms.addpars(  saturation=0.74,        year=year, unitcost=(3,7))
        SBCC.addpars(     saturation=0.6,         year=year, unitcost=(8,12))
        STI.addpars(      saturation=0.6,         year=year, unitcost=(15,20))
        VMMC.addpars(     saturation=(.5,.6),     year=year, unitcost=(15,25))
        FSW.addpars(      saturation=0.9,         year=year, unitcost=(25,35))
        MSM.addpars(      saturation=0.9,         year=year, unitcost=(25,35))
        PWID.addpars(     saturation=0.3,         year=year, unitcost=(25,35))
        OST.addpars(      saturation=0.3,         year=year, unitcost=(100,200))
        NSP.addpars(      saturation=0.3,         year=year, unitcost=(10,20))
        Cash.addpars(     saturation=0.3,         year=year, unitcost=(50,80))
        PrEP.addpars(     saturation=0.3,         year=year, unitcost=(100,200))
        HTC.addpars(      saturation=(0.85,0.95), year=year, unitcost=(5,10))
        ART.addpars(      saturation=0.99,        year=year, unitcost=(400,800))
        Lab.addpars(      saturation=0.99,        year=year, unitcost=(40,80))
        Adherence.addpars(saturation=0.99,        year=year, unitcost=(20,50))
        Tracing.addpars(  saturation=0.99,        year=year, unitcost=(20,50))
        PMTCT.addpars(    saturation=0.9,         year=year, unitcost=(100,200))
                                 
    if addcostcovdata:
        datayear = 2014
        Condoms.adddata(  year=datayear, spend=1e7, coverage=3e5)
        SBCC.adddata(     year=datayear, spend=1e7, coverage=3e5)
        STI.adddata(      year=datayear, spend=1e7, coverage=3e5)
        VMMC.adddata(     year=datayear, spend=1e7, coverage=3e5)
        FSW.adddata(      year=datayear, spend=8e6, coverage=240000)
        MSM.adddata(      year=datayear, spend=8e6, coverage=240000)
        PWID.adddata(     year=datayear, spend=2e6, coverage=25000)
        OST.adddata(      year=datayear, spend=2e6, coverage=25000)
        NSP.adddata(      year=datayear, spend=2e6, coverage=25000)
        Cash.adddata(     year=datayear, spend=2e6, coverage=25000)
        PrEP.adddata(     year=datayear, spend=2e6, coverage=25000)
        HTC.adddata(      year=datayear, spend=2e7, coverage=1.3e6)
        ART.adddata(      year=datayear, spend=1e6, coverage=3308)
        Lab.adddata(      year=datayear, spend=1e5, coverage=2000)
        Adherence.adddata(year=datayear, spend=1e5, coverage=2000)
        Tracing.adddata(  year=datayear, spend=2e1, coverage=2000)
        PMTCT.adddata(    year=datayear, spend=4e6, coverage=5500)

        OVC.adddata(       year=datayear, basespend=1e7)
        MGMT.adddata(      year=datayear, basespend=1e7)
        HR.adddata(        year=datayear, basespend=5e5)
        ENV.adddata(       year=datayear, basespend=1e7)
        SP.adddata(        year=datayear, basespend=1e7)
        ME.adddata(        year=datayear, basespend=1e7)
        INFR.adddata(      year=datayear, basespend=1e7)
        Other_care.adddata(year=datayear, basespend=1e7)
        Other.adddata(     year=datayear, basespend=5e5)
...
#        
#        # Fix up costs
#        R.programs['ART'].adddata(saturation=(0.9,0.9),
#                         year=year,
