...
def model(simpars, settings, verbose=2, safetymargin=0.8, benchmark=False):
    ...
    for t in range(npts): # Loop over time
        ...
        try: results.inci[:,t] = (newinfections)/float(dt)  # Store new infections AND new MTCT births
        except: raise Exception() #import traceback; traceback.print_exc(); import pdb; pdb.set_trace()
