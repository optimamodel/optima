...
def plotbycd4(results=None, whattoplot='people', figsize=(14,10), lw=2, titlesize=globaltitlesize, labelsize=globallabelsize, 
    ...
    hivstates = results.projectref().settings.hivstates
    ...
    for plt in range(nsims): # WARNING, copied from plotallocs()
        ...
        for s,state in enumerate(reversed(hivstates)): # Loop backwards so correct ordering -- first one at the top, not bottom
            if ismultisim: thisdata += results.raw[plt][ind][whattoplot][getattr(results.projectref().settings,state),:,:].sum(axis=(0,1))[indices] # If it's a multisim, need an extra index for the plot number
            else:          thisdata += results.raw[ind][whattoplot][getattr(results.projectref().settings,state),:,:].sum(axis=(0,1))[indices] # Get the best estimate
